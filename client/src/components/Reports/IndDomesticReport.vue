<template>
    <div>
        <v-app>
            <v-content>
                <v-container fluid > 
                    <h1> Individual Domestic Report</h1>
                    <v-select
                        :items="['250', '211']"
                        v-model="advisor"
                    ...
                    <v-simple-table dense>
                        <template v-slot:default>
                            ...
                            <tr>
                                ...
                                <td class="text-center" rowspan="3">Taxes (USD)</td>
                            ...
                            <tr v-for="transaction in transactions" :key="transaction.blank.uniqueNumber">
                                ...
                                <td><div v-if="transaction.paymentType === 'Cash'">{{(transaction.amount / transaction.conversionRate + transaction.taxLocal / transaction.conversionRate).toFixed(2)}}</div></td>
                                <td><div v-if="transaction.paymentType === 'Card'">{{(transaction.amount / transaction.conversionRate + transaction.taxLocal / transaction.conversionRate).toFixed(2)}}</div></td>
                                <td>{{(transaction.taxLocal / transaction.conversionRate).toFixed(2)}}</td>
                                <td>{{ ((transaction.amount / transaction.conversionRate) + (transaction.taxLocal / transaction.conversionRate)).toFixed(2)}}</td>
                            ...
                            <tr>
                                <td class="text-right" colspan="5">Total Nett Amount for Bank Remittence to "Air Via" (USD)</td>
        ...
        async init() {
            ...
            try{
                this.transactions = ((await TransactionsService.getIndDomReport(this.datemin + "!" + this.datemax)).data.transactions).filter(function (tr){
                    return tr.blank.advisor.uniqueNumber == vm.advisor &&
                        ['201', '101'].includes(tr.blank.uniqueNumber.slice(0,3));
            ...
            for(const tr of this.transactions){
                ...
                if(tr.paymentType === 'Cash'){
                    tot.cash = tot.cash + ((tr.amount / tr.conversionRate)) + ((tr.taxLocal / tr.conversionRate))
                } else {
                    tot.card = tot.card + ((tr.amount / tr.conversionRate)) + ((tr.taxLocal / tr.conversionRate))
                }

                tot.tax = tot.tax + ((tr.taxLocal / tr.conversionRate))

                tot.total = tot.total + ((tr.taxLocal / tr.conversionRate)) + ((tr.amount / tr.conversionRate))
